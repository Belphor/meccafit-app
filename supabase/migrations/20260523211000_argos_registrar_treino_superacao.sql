-- Restaura SUPERAÇÃO + payload na RPC (substitui versão void que quebrava o overlay)

DROP FUNCTION IF EXISTS public.registrar_treino_com_status(integer, text, text, numeric, integer, integer, uuid);
DROP FUNCTION IF EXISTS public.registrar_treino_com_status(uuid, text, numeric, date, text);
