-- FENYXIA · Mural comunidade — copy contextual (sem jargão ARGOS na UI)
...
AS $$
  SELECT
    ...
    format(
      'Novo recorde no %s com %s kg — a linhagem acompanha cada ascensão.',
      COALESCE(NULLIF(BTRIM(h.exercicio_nome), ''), 'treino'),
      to_char(COALESCE(h.peso, h.peso_atual, 0), 'FM9999990.0')
    ) AS topic_body,
