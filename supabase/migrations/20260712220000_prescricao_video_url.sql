-- Link de vídeo por exercício prescrito (URL externa — YouTube/Vimeo/etc.).
-- Sem Storage: só texto; o vídeo fica hospedado na internet (custo ~zero).

ALTER TABLE public.prescricoes_treino_forjador
  ADD COLUMN IF NOT EXISTS video_url text;

COMMENT ON COLUMN public.prescricoes_treino_forjador.video_url IS
  'URL externa do vídeo de execução (YouTube, Vimeo, etc.). Opcional.';
...
BEGIN
  ...
  IF v_video_url IS NOT NULL AND char_length(v_video_url) > 2000 THEN
    RAISE EXCEPTION 'invalid video_url' USING ERRCODE = '22023';
  END IF;

  IF v_video_url IS NOT NULL
     AND v_video_url !~* '^https?://'
  THEN
    RAISE EXCEPTION 'invalid video_url' USING ERRCODE = '22023';
  END IF;
