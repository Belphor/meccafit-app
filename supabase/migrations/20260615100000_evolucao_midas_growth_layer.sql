-- FENYXIA · MIDAS Growth Layer (planos · calendário · cargas · get_muscular_evolution)
-- Idempotente · sa-east-1 · schema mensal (total_treinos_mensais_planejados)
-- Compatível com planos_atletas já migrado (sem dias_treino_semana)
...
-- ---------------------------------------------------------------------------
-- 1. planos_atletas — metas dinâmicas do atleta (schema mensal · idempotente)
-- ---------------------------------------------------------------------------
...
-- Migra legado dias_treino_semana → mensal (se coluna antiga ainda existir)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
...
COMMENT ON FUNCTION public.get_muscular_evolution() IS
  'MIDAS · Evolução muscular JSONB · auth.uid · ignição mensal flexível (30d) · VTC/VRA 14d.';
