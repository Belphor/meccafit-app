-- VTC só no dia da planilha alinhado ao calendário civil de Brasília (servidor).
-- p_dia_planilha: 1=Seg … 6=Sáb. Se informado, deve coincidir com o dia SP atual.

DROP FUNCTION IF EXISTS public.registrar_treino_com_status(uuid, text, numeric, text, integer, integer, text);
...
CREATE OR REPLACE FUNCTION public.registrar_treino_com_status(
  ...
  p_dia_planilha integer DEFAULT NULL
)
RETURNS TABLE (
  status text,
...
BEGIN
  ...
  -- Calendário nacional (Brasília): domingo (0) mapeia para segunda (1) na forja.
  v_sp_dow := EXTRACT(DOW FROM timezone('America/Sao_Paulo', now()))::integer;
  v_planilha_hoje := CASE
    WHEN v_sp_dow = 0 THEN 1
    WHEN v_sp_dow = 6 THEN 6
    ELSE v_sp_dow
  END;

  IF p_dia_planilha IS NULL OR p_dia_planilha < 1 OR p_dia_planilha > 6 THEN
    RAISE EXCEPTION 'dia da planilha inválido'
      USING ERRCODE = '22023';
  END IF;

  IF p_dia_planilha <> v_planilha_hoje THEN
    RAISE EXCEPTION 'VTC só libera no dia da planilha alinhado ao calendário de Brasília'
      USING ERRCODE = 'P0001';
  END IF;
...
COMMENT ON FUNCTION public.registrar_treino_com_status(uuid, text, numeric, text, integer, integer, text, integer) IS
  '1 registro por exercício/dia SP. VTC exige dia da planilha = calendário Brasília. 2ª tentativa rejeitada.';

REVOKE ALL ON FUNCTION public.registrar_treino_com_status(uuid, text, numeric, text, integer, integer, text, integer)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.registrar_treino_com_status(uuid, text, numeric, text, integer, integer, text, integer)
