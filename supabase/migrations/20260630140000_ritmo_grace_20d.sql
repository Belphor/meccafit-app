-- Ritmo da Fênix · 20 dias de acolhimento sem penalidade visual no mapa corporal
...
DECLARE
  ...
  v_phase_setup_at timestamptz;
  v_days_since_setup integer;
...
BEGIN
  ...
  SELECT p.phase_setup_at, COALESCE(p.phase_tier, 1)::smallint
  INTO v_phase_setup_at, v_phase_tier
  ...
  v_days_since_setup := GREATEST(
    0,
    FLOOR(EXTRACT(EPOCH FROM (now() - COALESCE(v_phase_setup_at, now()))) / 86400)::integer
  );
  v_grace_active := v_days_since_setup < 20;
  v_grace_days_remaining := GREATEST(0, 20 - v_days_since_setup);
  ...
  v_degradacao := NOT v_grace_active AND v_ignicao < 50.0;
  ...
  RETURN jsonb_build_object(
    ...
    'ritmo_grace_active', v_grace_active,
    'ritmo_grace_days_remaining', v_grace_days_remaining,
...
COMMENT ON FUNCTION public.get_muscular_evolution() IS
  'MIDAS · Ritmo = VTC 30d / meta. Acolhimento 20d sem penalidade de mapa. Brasas = VTC 14d.';
