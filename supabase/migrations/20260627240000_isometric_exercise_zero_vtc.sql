-- Prancha / isométricos: PR em segundos, superação por tempo, vtc_gerado = 0.
-- Base: registrar_treino_com_status (20260529000000) + evolucao_is_isometric_core.
...
BEGIN
  ...
  v_vtc := CASE
    ...
    ELSE p_peso_atual * p_repeticoes * p_series
  END;
...
COMMENT ON FUNCTION public.registrar_treino_com_status(uuid, text, numeric, text, integer, integer, text) IS
  'Registra PR/superação. Isométricos (prancha): PR em segundos, vtc_gerado = 0.';
