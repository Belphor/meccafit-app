-- FENYXIA · Comunidade Mecca — integração automática em registrar_treino_com_status
-- Requer: 20260527240000_create_mecca_global_metrics.sql

-- ---------------------------------------------------------------------------
-- 1. Helper interno — UPDATE atómico (sem auth; só chamado por RPCs SECURITY DEFINER)
-- ---------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION public.mecca_apply_contribution_internal(p_weight_kg bigint)
RETURNS public.mecca_global_metrics
LANGUAGE plpgsql
VOLATILE
SECURITY DEFINER
...
AS $$
DECLARE
  v_row public.mecca_global_metrics;
  v_delta bigint;
BEGIN
  ...
  v_delta := LEAST(p_weight_kg, 99999);

  UPDATE public.mecca_global_metrics
  SET
    total_weight_lifted = total_weight_lifted + v_delta,
    furnace_temperature = public.mecca_compute_furnace_temperature(
      total_weight_lifted + v_delta
    ),
    updated_at = now()
...
REVOKE ALL ON FUNCTION public.mecca_apply_contribution_internal(bigint) FROM PUBLIC, anon, authenticated;

-- ---------------------------------------------------------------------------
-- 2. RPC pública — delega ao helper após validação ARGOS
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  v_metrics := public.mecca_apply_contribution_internal(p_weight_input::bigint);
...
-- ---------------------------------------------------------------------------
-- 3. registrar_treino_com_status — contribui VTC para a Comunidade Mecca (clientes)
-- ---------------------------------------------------------------------------
