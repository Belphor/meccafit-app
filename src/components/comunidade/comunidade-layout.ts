...
/** Tabs de filtro · scroll em mobile estreito · grid 3×2 / 6 colunas em desktop */
export const COMUNIDADE_TAB_LIST =
  "mt-4 flex w-full min-w-0 gap-1.5 overflow-x-auto pb-0.5 snap-x snap-mandatory [scrollbar-width:none] xs:gap-2 md:grid md:grid-cols-3 md:gap-2 md:overflow-visible md:pb-0 lg:grid-cols-6 [&::-webkit-scrollbar]:hidden";

export const COMUNIDADE_TAB_BUTTON =
  "flex min-h-11 min-w-[5.25rem] shrink-0 snap-center items-center justify-center rounded-full border px-2.5 text-center text-[10px] font-bold uppercase leading-tight tracking-[0.06em] transition-colors whitespace-normal md:min-w-0 md:w-full md:px-2 md:text-[9px] lg:text-[10px]";
